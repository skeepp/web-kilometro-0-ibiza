...
export function FollowButton({ producerId }: { producerId: string }) {
    ...
    if (isLoading) {
        return (
            <Button variant="outline" className="w-full sm:w-auto h-9 px-4 text-xs sm:text-sm font-semibold rounded-lg shadow-sm opacity-50" disabled>
                Cargando...
            </Button>
        );
    }
    ...
    return (
        <Button 
            ...
            className={`w-full sm:w-auto h-9 px-4 text-xs sm:text-sm font-semibold rounded-lg shadow-sm transition-all ${isFollowing ? 'border-brand-primary text-brand-primary hover:bg-gray-50' : 'bg-brand-primary text-white hover:bg-brand-primary/90 hover:-translate-y-0.5'}`}
